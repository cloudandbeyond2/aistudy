  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        if (response.data?.success) {
          setTestimonials(
            response.data.testimonials.filter((t: Testimonial) => t.approved)
          );
        }
      } catch (error) {
        console.error("Error fetching testimonials:", error);
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <div className="grid md:grid-cols-2 gap-8">
            {[1, 2, 3, 4].map((i) => (
              <Skeleton
                key={i}
                className="h-64 rounded-3xl bg-slate-200 dark:bg-slate-800"
              />
            ))}
          </div>
        </div>
        ...
        >
          {testimonials.map((testimonial) => (
            ...
            >
              ...
              {/* Message */}
              <p className="text-slate-600 dark:text-slate-300 italic text-xl leading-relaxed mb-8">
                "{testimonial.message}"
              </p>
