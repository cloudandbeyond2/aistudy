...
const CommunicationPractice = () => {
  ...
  const fetchScenario = async () => {
    ...
    try {
      const res = await axios.get(`${serverURL}/api/communication-practice/scenario?userId=${userId}&tone=${tone}`);
      if (res.data.success) setScenarioData(res.data.data);
    } catch (error) {
      toast({ title: 'Error', description: 'Failed to generate scenario', variant: 'destructive' });
    } finally {
      setGenerating(false);
  ...
  const submitScenario = async () => {
    ...
    try {
      const res = await axios.post(`${serverURL}/api/communication-practice/scenario`, {
        userId, scenario: scenarioData.scenario, userResponse
      });
      if (res.data.success) {
        setEvaluation(res.data.data.evaluation);
        setProfile((prev: any) => ({ ...prev, xp: prev.xp + res.data.data.xpAwarded, level: res.data.data.newLevel, streak: res.data.data.streak }));
        toast({ title: `+${res.data.data.xpAwarded} XP`, description: 'Scenario evaluated!' });
      }
    } catch (error) {
      toast({ title: 'Error', description: 'Failed to evaluate response', variant: 'destructive' });
  ...
  const fetchVocabulary = async () => {
    ...
    try {
      const res = await axios.get(`${serverURL}/api/communication-practice/vocabulary?userId=${userId}`);
      if (res.data.success) setVocabulary(res.data.data);
    } catch (error) {
      toast({ title: 'Error', description: 'Failed to generate vocabulary', variant: 'destructive' });
    } finally {
      setGenerating(false);
  ...
  const saveWord = async (wordData: any) => {
    try {
      const res = await axios.post(`${serverURL}/api/communication-practice/vocabulary`, { userId, ...wordData });
      if (res.data.success) {
        toast({ title: 'Saved', description: `${wordData.word} added to your bank. +2 XP` });
        setVocabulary(prev => prev.filter(v => v.word !== wordData.word));
        fetchProfile();
  ...
  const toggleMastery = async (wordId: string, currentStatus: boolean) => {
    try {
      const res = await axios.patch(`${serverURL}/api/communication-practice/vocabulary/mastery`, {
        userId, wordId, mastered: !currentStatus
      });
      if (res.data.success) {
        toast({ title: !currentStatus ? 'Word Mastered! +10 XP' : 'Moved to study list', description: `Status updated for ${res.data.data.word}` });
        fetchProfile();
      }
    } catch (error) {
      toast({ title: 'Error', description: 'Failed to update status', variant: 'destructive' });
  ...
  const deleteWord = async (wordId: string) => {
    try {
      const res = await axios.delete(`${serverURL}/api/communication-practice/vocabulary`, { data: { userId, wordId } });
      if (res.data.success) {
        toast({ title: 'Removed', description: 'Word removed from your bank' });
        fetchProfile();
      }
  ...
  const submitTest = async () => {
    ...
    try {
      const res = await axios.post(`${serverURL}/api/communication-practice/grammar-test`, {
        userId, score, totalQuestions: grammarTest.length
      });
      if (res.data.success) {
        setTestResult({ score, total: grammarTest.length, awarded: res.data.data.xpAwarded });
        setProfile((prev: any) => ({ ...prev, xp: prev.xp + res.data.data.xpAwarded, level: res.data.data.newLevel, streak: res.data.data.streak }));
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8 pt-20">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {/* ── HERO HEADER (Themed) ── */}
        <div className="rounded-2xl bg-brand-gradient p-6 md:p-8 shadow-xl text-white relative overflow-hidden">
          {/* decorative blobs */}
          <div className="absolute -top-10 -right-10 w-48 h-48 rounded-full bg-white/5 blur-2xl pointer-events-none" />
          <div className="absolute bottom-0 left-1/3 w-64 h-32 rounded-full bg-white/10 blur-3xl pointer-events-none" />
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Communication Practice</h1>
              </div>
              <p className="text-white/80 text-sm md:text-base max-w-lg leading-relaxed">
                Master professional language and communication with AI-driven scenarios, vocabulary flashcards, and grammar assessments.
              </p>
            </div>

            {/* Right: XP card + streak */}
            <div className="flex gap-3 shrink-0 w-full md:w-auto">
              <div className="flex-1 md:min-w-[200px] bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
                <p className="text-white/80 text-xs font-semibold mb-1">Level {profile?.level || 1} Communicator</p>
                <p className="text-2xl font-bold font-mono">{currentXP} <span className="text-sm font-normal text-white/70">XP</span></p>
                ...
                <p className="text-[10px] text-white/60 mt-1">Next level: {nextXP} XP</p>
              </div>

              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20 flex flex-col items-center justify-center min-w-[80px]">
                <Flame className={`h-7 w-7 ${(profile?.streak || 0) > 0 ? 'text-orange-400' : 'text-white/30'}`} />
                <span className="font-bold text-lg leading-none mt-1">{profile?.streak || 0}</span>
                <span className="text-[10px] text-white/70 uppercase font-semibold tracking-wider">Streak</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6">
            {[
              { icon: PenTool,   label: 'Scenario',   desc: 'Practice real-world communication with AI feedback.',    tab: 'scenario'    },
              { icon: BookOpen,  label: 'Vocabulary', desc: 'Build your word bank and master terms via flashcards.',   tab: 'vocabulary'  },
              { icon: Target,    label: 'Tests',      desc: 'Assess grammar and syntax with AI-generated questions.', tab: 'grammar'     },
            ].map(({ icon: Icon, label, desc, tab }) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className="text-left p-4 rounded-xl bg-white/10 hover:bg-white/20 border border-white/15 transition-all duration-200 group"
              >
                <div className={`w-9 h-9 rounded-lg bg-white/10 border border-white/20 flex items-center justify-center mb-3`}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsList className="flex w-full overflow-x-auto bg-white border border-slate-200 p-1 rounded-xl shadow-sm h-auto gap-1 no-scrollbar">
            {[
              { value: 'dashboard', icon: History,     label: 'Hub'        },
              { value: 'scenario',  icon: PenTool,     label: 'Scenario'   },
              { value: 'vocabulary',icon: BookOpen,    label: 'Vocabulary' },
              { value: 'grammar',   icon: Target,      label: 'Tests'      },
            ].map(({ value, icon: Icon, label }) => (
              <TabsTrigger
                key={value}
                value={value}
                className="flex-1 min-w-[90px] py-2.5 px-3 flex items-center justify-center gap-2 text-sm font-medium rounded-lg whitespace-nowrap
          ...
          <TabsContent value="dashboard" className="mt-6 space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs font-bold text-slate-700">
                          {profile?.vocabularyBank?.filter((v: any) => v.mastered).length || 0}/{profile?.vocabularyBank?.length || 0}
                        </span>
                      </div>
                    </div>
                  </CardTitle>
                ...
                <CardContent>
                  {profile?.practiceLogs?.length > 0 ? (
                    <div className="space-y-3">
                      {profile.practiceLogs.slice(0, 5).map((log: any, i: number) => (
                        <div key={i} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50/50 gap-3">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className={`h-10 w-10 shrink-0 rounded-full flex items-center justify-center ${log.score > 80 ? 'bg-emerald-100 text-emerald-600' : 'bg-primary/10 text-primary'}`}>
                              <CheckCircle className="h-5 w-5" />
                            </div>
                            <div className="min-w-0">
                              <p className="text-sm font-bold text-slate-800 truncate">{log.scenario}</p>
                              <p className="text-[10px] text-slate-500 uppercase font-semibold">{log.date}</p>
                            </div>
                          </div>
                          <div className="text-right shrink-0">
                            <p className="text-sm font-bold text-primary">+{log.xpAwarded} XP</p>
                            <p className="text-[10px] text-slate-400">{log.score}/100</p>
                          </div>
                        </div>
                      ))}
              ...
              <div className="space-y-4">
                {/* Tip Card */}
                <Card className="border-0 shadow-md bg-brand-gradient text-white overflow-hidden relative">
                  <div className="absolute top-0 right-0 p-4 opacity-10">
                    <Lightbulb className="h-20 w-20" />
                  </div>
                  <CardHeader>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="text-[10px] font-bold uppercase tracking-wider border border-white/20 px-2 py-0.5 rounded-full bg-white/10 w-fit">
                      {DAILY_TIPS[new Date().getDate() % DAILY_TIPS.length].title}
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-0 shadow-sm">
                  ...
                  <CardContent className="p-2 space-y-1">
                    <Button variant="ghost" className="w-full justify-start text-slate-700 hover:bg-primary/5 hover:text-primary transition-colors" onClick={() => setActiveTab('scenario')}>
                      <PenTool className="h-4 w-4 mr-2" /> Start Scenario
                    </Button>
                    <Button variant="ghost" className="w-full justify-start text-slate-700 hover:bg-primary/5 hover:text-primary transition-colors" onClick={() => setActiveTab('vocabulary')}>
                      <BookOpen className="h-4 w-4 mr-2" /> Expand Vocab
                    </Button>
                    <Button variant="ghost" className="w-full justify-start text-slate-700 hover:bg-primary/5 hover:text-primary transition-colors" onClick={() => setActiveTab('grammar')}>
                      <Target className="h-4 w-4 mr-2" /> Take Test
                    </Button>
                  </CardContent>
                </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="border-0 shadow-sm">
                <CardHeader className="bg-primary/5 border-b border-primary/10 pb-4">
                  <CardTitle className="text-lg flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <span className="flex items-center gap-2">
                      <Sparkles className="h-5 w-5 text-primary" /> Scenario Generator
                    </span>
                    <div className="flex items-center gap-2">
                      <Select value={tone} onValueChange={setTone}>
                      ...
                      <Button onClick={fetchScenario} disabled={generating} size="sm" className="bg-brand-gradient text-white hover:opacity-90 shrink-0">
                  ...
                  {scenarioData ? (
                    <div className="space-y-4">
                      ...
                      <Textarea
                        ...
                        onChange={(e) => setUserResponse(e.target.value)}
                      />
                      <Button onClick={submitScenario} disabled={evaluating || !userResponse.trim()} className="w-full bg-brand-gradient text-white hover:opacity-90">
                        {evaluating ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
                    ...
                    <div className="h-40 flex items-center justify-center text-slate-400 flex-col">
                      <MessageSquare className="h-8 w-8 mb-2 opacity-20" />
                      <p className="text-sm text-center">Click Generate to begin your daily practice.</p>
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              {evaluation && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                  <Card className="border-0 shadow-lg bg-gradient-to-br from-slate-800 to-slate-900 text-white h-full relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-8 opacity-10 rotate-12">
                      <Zap className="h-32 w-32" />
                    </div>
                    <CardHeader>
                      <CardTitle className="text-primary-foreground/80">AI Evaluation</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-5 relative z-10">
                      <div className="flex items-end gap-3 border-b border-white/10 pb-4">
                        <span className="text-5xl font-bold">{evaluation.score}</span>
                        <span className="text-primary-foreground/60 pb-1 font-medium">/ 100</span>
                      </div>
                      <div>
                        <h4 className="text-xs font-semibold text-primary-foreground/60 uppercase tracking-wider mb-2">Feedback</h4>
                        <p className="text-slate-200 leading-relaxed text-sm bg-white/5 p-4 rounded-lg">{evaluation.feedback}</p>
                      </div>
                      <div>
                        <h4 className="text-xs font-semibold text-emerald-400 uppercase tracking-wider mb-2">Improved Version</h4>
                        <p className="text-emerald-50 leading-relaxed text-sm bg-emerald-900/40 p-4 rounded-lg border border-emerald-500/30">
                          {evaluation.improvedVersion}
                        </p>
                      </div>
...
>
  ...
  {isStudyMode ? "Exit Study" : "Flashcards"}
</Button>
                  <Button onClick={fetchVocabulary} disabled={generating} className="bg-brand-gradient text-white hover:opacity-90">
                ...
                <div className="space-y-6">
                  ...
                  {vocabulary.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {vocabulary.map((v, i) => (
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} key={i}>
                          <Card className="h-full border-0 shadow-md hover:shadow-lg transition-shadow relative overflow-hidden flex flex-col">
                            <div className="absolute top-0 left-0 w-1 h-full bg-primary" />
                            <CardHeader className="pb-2">
                              <CardTitle className="text-xl capitalize text-primary">{v.word}</CardTitle>
                            </CardHeader>
                            <CardContent className="flex flex-col flex-1">
                              <p className="text-slate-700 mb-3 flex-1">{v.meaning}</p>
                              <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 mb-3 max-h-20 overflow-y-auto">
                                <p className="text-sm text-slate-600 italic">"{v.example}"</p>
                              </div>
                              <button
                                onClick={() => saveWord(v)}
                                className="w-full mt-auto py-2 px-4 rounded-md border border-primary/20 text-primary text-sm font-medium bg-white hover:bg-brand-gradient hover:text-white hover:border-transparent transition-colors duration-200"
                              >
                                Save to Bank
                              </button>
                            </CardContent>
                        ...
                        {profile.vocabularyBank
                          ...
                          <div key={v._id || i} className={`p-4 rounded-xl border transition-all ${v.mastered ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-200'}`}>
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div className="flex gap-1">
                                <button
                                  onClick={() => toggleMastery(v._id, v.mastered)}
                                  className={`p-1 rounded-md transition-colors ${v.mastered ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-slate-100 text-slate-400 hover:bg-emerald-100 hover:text-emerald-600'}`}
                                >
                                  <Check className="h-3 w-3" />
                                </button>
                                <button
                                  onClick={() => deleteWord(v._id)}
                                  className="p-1 rounded-md bg-slate-100 text-slate-400 hover:bg-red-100 hover:text-red-600 transition-colors"
                                >
                                  <Trash2 className="h-3 w-3" />
                                </button>
                              </div>
                ...
                <div className="flex flex-col items-center py-8 px-4">
                  {profile?.vocabularyBank?.filter((v: any) => !v.mastered).length > 0 ? (
                    <div className="w-full max-w-md space-y-6">
                      <div className="text-center">
                        <p className="text-sm font-bold text-primary uppercase tracking-widest mb-1">
                          Flashcard {flashcardIndex + 1} of {profile.vocabularyBank.filter((v: any) => !v.mastered).length}
                        </p>
                        <p className="text-xs text-slate-400">Click card to flip</p>
                      </div>

                      <div className="perspective-1000 h-72 sm:h-80 relative cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
                        <motion.div
                          className="w-full h-full relative"
                          initial={false}
                          animate={{ rotateY: isFlipped ? 180 : 0 }}
                          ...
                          style={{ transformStyle: "preserve-3d" }}
                        >
                          <div className="absolute w-full h-full bg-white rounded-3xl border-2 border-primary/20 shadow-xl flex flex-col items-center justify-center p-8 backface-hidden">
                            <h4 className="text-3xl sm:text-4xl font-black text-slate-900 capitalize text-center">
                              {profile.vocabularyBank.filter((v: any) => !v.mastered)[flashcardIndex]?.word}
                            </h4>
                            <div className="mt-8 flex items-center text-primary gap-2">
                              <RotateCcw className="h-4 w-4" />
                              <span className="text-xs font-bold uppercase">Click to view meaning</span>
                            </div>
                          </div>
                          <div className="absolute w-full h-full bg-brand-gradient rounded-3xl text-white flex flex-col items-center justify-center p-8 backface-hidden" style={{ transform: "rotateY(180deg)" }}>
                            <div className="text-center space-y-4">
                              <div>
                                <p className="text-[10px] uppercase font-black text-white/70 tracking-tighter mb-1">Meaning</p>
                                <p className="text-base font-medium leading-tight">
                                  {profile.vocabularyBank.filter((v: any) => !v.mastered)[flashcardIndex]?.meaning}
                                </p>
                              </div>
                              <div className="pt-4 border-t border-white/10">
                                <p className="text-[10px] uppercase font-black text-white/70 tracking-tighter mb-1">Example</p>
                                <p className="text-sm italic opacity-90">
                                  "{profile.vocabularyBank.filter((v: any) => !v.mastered)[flashcardIndex]?.example}"
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm w-full max-w-md">
                      ...
                      <h4 className="text-xl font-bold text-slate-900">All caught up!</h4>
                      <p className="text-slate-500 mt-2 text-sm px-4">You've mastered all the words in your bank.<br />Add new words to continue practicing.</p>
                      <Button variant="link" className="mt-4 text-primary" onClick={() => setIsStudyMode(false)}>Back to Bank</Button>
                    </div>
                  )}
                </div>
              )}
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader className="bg-primary/5 border-b border-primary/10">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <CardTitle className="text-lg text-primary">Grammar & Syntax Assessment</CardTitle>
                    <CardDescription>Test your mechanics with AI-generated questions.</CardDescription>
                  </div>
                  {!grammarTest.length && (
                    <Button onClick={generateTest} disabled={generating} className="bg-brand-gradient text-white hover:opacity-90 shrink-0">
              ...
              <CardContent className="pt-6">
                {grammarTest.length > 0 ? (
                  <div className="space-y-6">
                    {grammarTest.map((q, qIndex) => (
                      <div key={qIndex} className="bg-white border border-slate-200 rounded-xl p-4 sm:p-5 shadow-sm">
                        <p className="font-medium text-slate-800 mb-4 text-sm sm:text-base">
                          <span className="text-primary font-bold mr-2">{qIndex + 1}.</span>{q.question}
                        </p>
                        <div className="space-y-2">
                          {q.options.map((opt: string, oIndex: number) => {
                            const isSelected = testAnswers[qIndex] === oIndex;
                            ...
                            let btnClass = "w-full text-left py-3 px-4 rounded-lg border text-sm font-normal transition-colors duration-150 flex items-center justify-between gap-2 ";

                            if (testResult) {
                              if (isCorrect) btnClass += "bg-emerald-50 border-emerald-400 text-emerald-800 font-medium cursor-default";
                              else if (isWrong) btnClass += "bg-red-50 border-red-400 text-red-800 font-medium cursor-default";
                              else btnClass += "bg-white border-slate-200 text-slate-400 opacity-60 cursor-default";
                            } else if (isSelected) {
                              btnClass += "bg-primary/10 border-primary text-primary font-medium shadow-sm";
                            } else {
                            ...
                            return (
                              ...
                              >
                                <span>{opt}</span>
                                {testResult && isCorrect && <CheckCircle className="h-4 w-4 text-emerald-600 shrink-0" />}
                              </button>
                            );
                          })}
                        </div>
                    ...
                    <div className="flex justify-end pt-4 border-t border-slate-100">
                      {!testResult ? (
                        <Button
                          ...
                          onClick={submitTest}
                          disabled={evaluating}
                          className="bg-brand-gradient text-white hover:opacity-90 w-full sm:w-auto"
                        >
                          {evaluating ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
                          {evaluating ? 'Submitting...' : 'Submit Assessment'}
                        </Button>
                      ) : (
                        <Button size="lg" onClick={generateTest} disabled={generating} className="w-full sm:w-auto">
                          Take Another Test
                        </Button>
                      )}
                    </div>
                  ...
                  <div className="h-40 flex items-center justify-center text-slate-400 flex-col">
                    <Target className="h-8 w-8 mb-2 opacity-20" />
                    <p className="text-sm">{generating ? 'Generating assessment...' : 'Click Start to test your grammar skills.'}</p>
                  </div>
                )}
              </CardContent>
            </Card>
