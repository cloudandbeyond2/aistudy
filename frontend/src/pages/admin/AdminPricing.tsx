...
const AdminPricing = () => {
  ...
  const [selectedPlan, setSelectedPlan] =
    useState<'free' | 'monthly' | 'yearly'>('monthly');
  const [hasChanges, setHasChanges] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      await axios.post(`${serverURL}/api/admin/pricing/update`, {
        pricing: pricingData,
      });
      setHasChanges(false);
      setSubmitStatus('success');
  ...
  const handleReset = () => {
    setPricingData((prev) => ({
      ...
      [selectedPlan]: {
        planType: selectedPlan,
        planName: PLAN_CONFIGS[selectedPlan].name,
        price:
          selectedPlan === 'free'
            ? 0
            ...
            : 99,
        currency: 'USD',
        billingPeriod: PLAN_CONFIGS[selectedPlan].period,
      },
    }));
    setHasChanges(false);
  };
    ...
    <div className="space-y-6">
      ...
      {submitStatus === 'success' && (
        <Alert className="border-green-200 bg-green-50">
          <CheckCircle className="h-4 w-4 text-green-600" />
          <AlertDescription>Pricing updated successfully</AlertDescription>
        </Alert>
      )}
      ...
      {submitStatus === 'error' && (
        <Alert className="border-red-200 bg-red-50">
          <AlertCircle className="h-4 w-4 text-red-600" />
          <AlertDescription>Update failed</AlertDescription>
        </Alert>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input value={currentPricing.planName} onChange={handlePlanNameChange} />
            </div>
