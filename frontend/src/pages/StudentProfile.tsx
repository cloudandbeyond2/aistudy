...
const handleChange = (e) => {
  ...
  // ✅ Phone
  if (!formData.phone) {
  toast({ title: "Phone required", description: "Enter phone number" });
  return false;
}
...
if (!phoneRegex.test(formData.phone)) {
  ...
  // ✅ DOB
  if (!formData.dob) {
    toast({ title: "DOB required", description: "Select DOB" });
    return false;
  }
  ...
  if (!formData.country.trim()) {
    toast({ title: "Country required" });
    return false;
  }

  // ✅ State
  if (!formData.state.trim()) {
    toast({ title: "State required" });
    return false;
  }

  // ✅ City
  if (!formData.city.trim()) {
    toast({ title: "City required" });
    return false;
  }
  ...
  if (!pinRegex.test(formData.pin)) {
    toast({ title: "Invalid PIN", description: "Enter 6 digits" });
    return false;
  }
