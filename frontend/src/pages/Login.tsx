...
const Login = () => {
  ...
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');
  const [stats, setStats] = useState({ totalUsers: 0, totalCourses: 0 }); // Added for live counts
  const navigate = useNavigate();
  const { toast } = useToast();
  ...
  // Fetch Live Stats for the Landing Panel
  useEffect(() => {
    ...
    <div className="flex min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-primary via-primary/90 to-indigo-600 relative items-center justify-center p-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg text-white">
          ...
          >
            <div className="bg-white/10 backdrop-blur-sm p-4 rounded-2xl border border-white/10">
              <BookOpen className="h-6 w-6 mb-2 text-white/60" />
