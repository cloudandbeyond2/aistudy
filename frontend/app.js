...
const certificateSettingsSchema = new mongoose.Schema({
    ...
    qrCodeUrl: { type: String, default: 'https://aicourse.agli.in/verify-certificate' }
});
...
app.post('/api/image', async (req, res) => {
    ...
    function logResults(error, results) {
        if (error) {
            //ERROR
            console.log('Error', error);
            res.status(500).json({ success: false, message: 'Image search failed: ' + (error.message || String(error)) });
        }
        else {
...
app.post('/api/course', async (req, res) => {
    ...
    // Default placeholder image in case Unsplash fails
    const defaultPhoto = 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800';

    // Try to get photo from Unsplash, but don't fail if it doesn't work
    try {
        ...
        const photo = (photos && photos.length > 0) ? photos[0].urls.regular : defaultPhoto;
        ...
        // If Unsplash fails, use default photo and continue
        console.log('Unsplash API error, using default photo:', error.message);
        try {
            const newCourse = new Course({ user, content, type, mainTopic, photo: defaultPhoto });
            await newCourse.save();
            const newLang = new LangSchema({ course: newCourse._id, lang: lang });
            await newLang.save();
            res.json({ success: true, message: 'Course created successfully', courseId: newCourse._id });
...
//UPDATE RESULT AND COMPLETE COURSE
app.post('/api/updateresult', async (req, res) => {
    ...
    try {
        ...
        // User lookup handled safely below
        // We need to fetch user details correctly. The course schema has 'user' field which is likely the user's email or ID. 
        // Let's check course schema again. It says 'user: String'. 
        // If it's email, we need to find user by email. If ID, by ID.
        // Based on other endpoints, it seems 'user' in Course might be email or ID. 
        // Let's assume we can get user details. 
        // Actually, let's just use the user string from course for now, or better, fetch user to get name.

        // Let's find the user. If course.user is email:
        let studentName = 'Student';
        let userId = course.user;
        const userObj = await User.findOne({ $or: [{ _id: (mongoose.Types.ObjectId.isValid(course.user) ? course.user : null) }, { email: course.user }] });
        if (userObj) {
        ...
        let issuedCert = await IssuedCertificate.findOne({ user: userId, course: courseId });
        if (!issuedCert) {
        ...
        res.json({ success: true, message: 'Result updated and course completed', certificateId: issuedCert.certificateId });
...
app.post('/api/getmyresult', async (req, res) => {
    const { courseId } = req.body;
    try {
        const exam = await ExamSchema.findOne({ course: courseId, passed: true });
        ...
        if (exam) {
            // Find the user associated with the course to retrieve certificate
            ...
            if (userId) {
                const cert = await IssuedCertificate.findOne({ course: courseId, user: userId });
                if (cert) certificateId = cert.certificateId;
            }

            res.json({ success: true, message: true, lang: language, certificateId: certificateId });
        } else {
            res.json({ success: true, message: false, lang: language });
        }
...
app.get('/api/verify-certificate/:id', async (req, res) => {
    try {
        const certificate = await IssuedCertificate.findOne({ certificateId: req.params.id });
        if (certificate) {
...
app.get('/api/courses', async (req, res) => {
    try {
        ...
        const coursesWithCert = await Promise.all(courses.map(async (course) => {
            if (course.completed) {
                const cert = await IssuedCertificate.findOne({ user: userId, course: course._id });
                if (cert) {
...
app.post('/api/aiexam', async (req, res) => {
    ...
    // Construct prompt for 10 MCQs
    // Construct prompt for 10 MCQs with specific JSON structure for frontend
    const prompt = `Generate 10 multiple choice questions about ${mainTopic} including these subtopics: ${subtopicsString}. The language should be ${lang}. Return ONLY a raw JSON array (no markdown, no code blocks) with this specific format:
    [
      {
        "id": 1,
    ...
    IMPORTANT: "correctAnswer" must match the "id" of the correct option exactly.`;
    ...
    try {
        ...
        console.log("Error generating exam:", error);
...
//ADMIN UPDATE COURSE
app.post('/api/admin/updatecourse', async (req, res) => {
    const { courseId, completed, restricted } = req.body;
    try {
        await Course.findOneAndUpdate(
            { _id: courseId },
            { $set: { completed: completed, restricted: restricted } }
        ).then(result => {
            res.json({ success: true, message: 'Course updated successfully' });
        }).catch(error => {
...
app.get('/api/certificate-settings', async (req, res) => {
    try {
...
app.post('/api/admin/certificate-settings', async (req, res) => {
    const { ceoName, ceoSignature, vpName, vpSignature, logo, qrCodeUrl } = req.body;
    try {
        let settings = await CertificateSettings.findOne();
        if (!settings) {
            settings = new CertificateSettings({ ceoName, ceoSignature, vpName, vpSignature, logo, qrCodeUrl });
        } else {
        ...
        await settings.save();
        res.json({ success: true, message: 'Certificate settings updated successfully' });
    } catch (error) {
...
app.get('/api/getpaid', async (req, res) => {
    try {
        ...
        const usersWithSubs = await Promise.all(paidUsers.map(async (user) => {
...
app.post('/api/admin/updateuser', async (req, res) => {
    const { userId, mName, email, type } = req.body;
    try {
...
app.post('/api/admin/upgrade-user', async (req, res) => {
    ...
    try {
        ...
        // Update user type to 'forever' (premium/lifetime)
        user.type = 'forever';
        await user.save();
        ...
        console.log('Error upgrading user:', error);
        res.status(500).json({ success: false, message: 'Internal server error' });
    }
});
